# furniture-app-kashta-kala-self employment
An Android furniture shopping app developed using Kotlin and XML that allows users to browse products, manage cart and wishlist, and place orders with a simple and user-friendly interface.<img width="527" height="832" alt="Screenshot 2026-05-18 152308" src="https://github.com/user-attachments/assets/8ff7ca1b-9807-41ce-9ef2-75f2b71a43ce" />
<img width="527" height="832" alt="Screenshot 2026-05-18 152308" src="https://github.com/user-attachments/assets/49ec8445-ea52-43f8-8466-8de8b8c92fe3" />
<img width="1920" height="1080" alt="Screenshot 2026-05-18 152121" src="https://github.com/user-attachments/assets/65e94354-11e0-4738-80ae-d966e3a53f9b" />

Kashta Kala is a modern furniture shopping mobile application designed to help users explore, view, and manage furniture products with a clean and user-friendly interface. The app focuses on providing a smooth browsing experience for different furniture categories such as sofas, chairs, tables, beds, and home décor items.

## Features

- User-friendly UI/UX
- Browse furniture products
- Category-wise product listing
- Product detail view
- Modern mobile app design
- Responsive layouts
- Clean navigation system
- Attractive furniture showcase

## Technologies Used

- Android Studio
- Java / Kotlin
- XML
- Firebase
- RecyclerView
- Material Design

## Problem Statement

Traditional furniture shopping can be time-consuming and inconvenient. Kashta Kala simplifies the process by providing a digital platform where users can easily browse furniture collections, explore product details, and experience a modern shopping interface from their mobile devices.

## Project Objective

The main objective of this project is to create an easy-to-use furniture application with an attractive design that improves user experience and simplifies furniture browsing and selection.

## Folder Structure

```bash
app/
├── java/
├── res/
├── manifests/
